...
class UpgraderTest < ActiveSupport::TestCase
  ...
  setup do
    ...
    Sencash.stubs(:version).returns(CURRENT_VERSION)
    Sencash.stubs(:commit_sha).returns(CURRENT_COMMIT)
  ...
  test "when app is behind latest version and latest commit is ahead of release finds release upgrade and no completed upgrades" do
    Sencash.stubs(:version).returns(PRIOR_VERSION)
    Sencash.stubs(:commit_sha).returns(PRIOR_COMMIT)
